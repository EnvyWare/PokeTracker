...
public class PokemonCatchListener {
    ...
    public void onBattleEnd(BattleEndEvent event) {
        Pair<ServerPlayer, List<PixelmonEntity>> pair = this.wasWildDefeated(event);
    ...
    private Pair<ServerPlayer, List<PixelmonEntity>> wasWildDefeated(BattleEndEvent event) {
        ...
        for (Map.Entry<BattleParticipant, BattleResults> entry : event.getResults().entrySet()) {
            if (entry.getKey() instanceof WildPixelmonParticipant) {
                ...
                player = (ServerPlayer) entry.getKey().getEntity();
